...
export default function AdminPage() {
  ...
  const addBlackout = async (event: FormEvent) => {
    ...
    setBlackout({ startsAt: "", endsAt: "", reason: "" }); await load();
  };
  ...
  return (
    <section className="admin-page">
      ...
      {!data ? <div className="state-card">Loading private booking details…</div> : <>
        ...
        {tab === "hours" && <div className="settings-card"><h2>Weekly hours</h2>{[...data.hours].sort((a, b) => a.dayOfWeek - b.dayOfWeek).map((row) => <div className="hours-row" key={row.id}><strong>{DAYS[row.dayOfWeek - 1]}</strong><label><span>Open</span><input type="time" step="1800" value={row.opensAt} disabled={row.isClosed} onChange={(e) => setData({ ...data, hours: data.hours.map((item) => item.id === row.id ? { ...item, opensAt: e.target.value } : item) })} /></label><label><span>Close</span><input type="time" step="1800" value={row.closesAt} disabled={row.isClosed} onChange={(e) => setData({ ...data, hours: data.hours.map((item) => item.id === row.id ? { ...item, closesAt: e.target.value } : item) })} /></label><label className="checkbox"><input type="checkbox" checked={row.isClosed} onChange={(e) => setData({ ...data, hours: data.hours.map((item) => item.id === row.id ? { ...item, isClosed: e.target.checked } : item) })} /> Closed</label></div>)}<button className="button primary" onClick={() => void saveHours()}>Save weekly hours</button></div>}
        {tab === "blackouts" && <div className="settings-grid"><form className="settings-card" onSubmit={addBlackout}><h2>Add one-time unavailable time</h2><p>For a repeating hold, use “Add event or hold” above.</p><label>Starts<input type="datetime-local" step="1800" value={blackout.startsAt} onChange={(e) => setBlackout({ ...blackout, startsAt: e.target.value })} required /></label><label>Ends<input type="datetime-local" step="1800" value={blackout.endsAt} onChange={(e) => setBlackout({ ...blackout, endsAt: e.target.value })} required /></label><label>Reason<input value={blackout.reason} maxLength={200} onChange={(e) => setBlackout({ ...blackout, reason: e.target.value })} required /></label><button className="button primary">Block this time</button></form><div className="settings-card"><h2>Upcoming holds</h2>{data.blackouts.map((item) => <div className="blackout-row" key={item.id}><div><strong>{item.reason} {item.seriesId && <span className="status-pill series">Series</span>}</strong><span>{formatDay(item.startsAt)}, {formatTime(item.startsAt)}–{formatTime(item.endsAt)}</span></div><div className="card-actions"><button className="button ghost small" onClick={() => setWizard({ initial: draftFromHold(item) })}>Edit</button><button className="button danger small" onClick={() => askRemove(item.id, item.reason, Boolean(item.seriesId))}>Remove</button></div></div>)}</div></div>}
