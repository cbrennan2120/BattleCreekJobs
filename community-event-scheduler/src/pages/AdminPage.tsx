...
interface AuditRow { id: string; action: string; entityType: string; actorLabel?: string | null; ipAddress?: string | null; metadata?: unknown; createdAt: string }
  ...
  return (
    <section className="admin-page">
      ...
      {!data ? <div className="state-card">Loading private booking details…</div> : <>
        ...
        {tab === "audit" && <div className="settings-card" id="admin-panel-audit" role="tabpanel" aria-labelledby="admin-tab-audit"><h2>Recent staff and booking activity</h2><table><thead><tr><th>When</th><th>Action</th><th>Type</th><th>Actor</th><th>IP address</th></tr></thead><tbody>{data.audit.map((row) => <tr key={row.id}><td>{new Date(row.createdAt).toLocaleString()}</td><td>{row.action}</td><td>{row.entityType}</td><td>{row.actorLabel || "System"}</td><td className="ip-address">{row.ipAddress || "Not recorded"}</td></tr>)}</tbody></table></div>}
      </>}
